...
bool input_select() {
    return callback_button(6, sf::Keyboard::H);
}
bool input_start() {
    return callback_button(7, sf::Keyboard::J);
}
...
class App {
  public:
    ...
    struct SaveState {
        SaveState(const Cartridge& c, const MMU& m, const LR35902& lr, const GPU& g) : cartridge(c), mmu(m), cpu(lr), gpu(g) {}
        Cartridge cartridge;
        MMU       mmu;
        LR35902   cpu;
        GPU       gpu;
    ...
    std::deque<SaveState>::iterator current_rewind_frame = save_states.end();
    ...
    int main(int argc, char* argv[]) {
        ...
        while(window.isOpen()) {
                ...
                if(with_sound) {
                    ...
                    if(samples_count > 0) {
                        if(samples_count >= snd_buffer.buffer_size) {
                            ...
                            blip_sample_t discard[2048]; // snd_buffer.chunk_size
                        ...
                        if(!(snd_buffer.getStatus() == sf::SoundSource::Status::Playing))
                            snd_buffer.play();
                    }
                }
            ...
            float scale = std::min(win_size.x / sprite_lb.width, win_size.y / sprite_lb.height);
            scale = std::max(scale, 1.0f);
            gameboy_screen_sprite.setScale(scale, scale);
            auto sprite_gb = gameboy_screen_sprite.getGlobalBounds();
            gameboy_screen_sprite.setPosition(win_size.x / 2 - sprite_gb.width / 2, win_size.y / 2 - sprite_gb.height / 2);
            if(show_gui)
                gameboy_screen_sprite.setColor(sf::Color{255, 255, 255, 128});
            else
                gameboy_screen_sprite.setColor(sf::Color::White);
    ...
    std::filesystem::path explore(const std::filesystem::path& path, const std::vector<const char*>& extensions = {}) {
    ...
    void gui() {
        ...
        if(ImGui::BeginMainMenuBar()) {
            ...
            if(ImGui::BeginMenu("Experimental")) {
                ImGui::Text("Rewind");
                if(ImGui::MenuItem("Rewind One Frame", "V"))
                    rewind_previous_frame();
                if(ImGui::MenuItem("Forward One Frame", "B"))
                    rewind_next_frame();
                if(ImGui::MenuItem("Resume", "C"))
                    stop_rewind();
        ...
        if(debug_window) {
            ...
            if(ImGui::CollapsingHeader("GPU")) {
                ImGui::Text("LY: 0x%02X, LCDC: 0x%02X, STAT: 0x%02X, Scroll: %d, %d", gpu.get_line(), gpu.get_lcdc(), gpu.get_lcdstat(), gpu.get_scroll_x(), gpu.get_scroll_y());
                ImGui::Text("LCDC: BGDisplay %d, OBJDisplay %d, OBJSize %d, BGTileMapDisplaySelect %d", gpu.get_lcdc() & GPU::BGDisplay, gpu.get_lcdc() & GPU::OBJDisplay,
                            gpu.get_lcdc() & GPU::OBJSize, gpu.get_lcdc() & GPU::BGTileMapDisplaySelect);
                ImGui::Text("      BGWindowsTileDataSelect %d, WindowDisplay %d, WindowsTileMapDisplaySelect %d, LCDDisplayEnable %d",
                            gpu.get_lcdc() & GPU::BGWindowsTileDataSelect, gpu.get_lcdc() & GPU::WindowDisplay, gpu.get_lcdc() & GPU::WindowsTileMapDisplaySelect,
            ...
            if(ImGui::CollapsingHeader("Breakpoints")) {
                ...
                if(ImGui::Button("Add breakpoint") && strlen(addr_buff) > 0) {
                    ...
                    } catch(std::exception& e) { log("Exception: '", e.what(), "', raised while adding a breakpoint."); }
                }

                static int                      selected_breakpoint = 0;
                static std::vector<std::string> labels;
                ImGui::ListBox(
                    "Breakpoints", &selected_breakpoint,
                    [](void* cpu, int idx, const char** out_text) -> bool {
                        if(labels.size() < static_cast<size_t>(idx + 1))
                            labels.resize(idx + 1);
                        auto addr = static_cast<LR35902*>(cpu)->get_breakpoints()[idx];
                        labels[idx] = Hexa(addr).str() + " (" + static_cast<LR35902*>(cpu)->get_disassembly(addr) + ")";
                        out_text[0] = labels[idx].c_str();
                        return true;
                    },
                    &cpu, cpu.get_breakpoints().size());
            ...
            if(ImGui::CollapsingHeader("Memory")) {
                ...
                while(clipper.Step()) {
                    ...
                    for(int i = clipper.DisplayStart; i < clipper.DisplayEnd; ++i) {
                        ImGui::Text("0x%04X: %02X %02X %02X %02X   %02X %02X %02X %02X   %c%c%c%c%c%c%c%c", addr, mmu.read(addr), mmu.read(addr + 1), mmu.read(addr + 2),
                                    mmu.read(addr + 3), mmu.read(addr + 4), mmu.read(addr + 5), mmu.read(addr + 6), mmu.read(addr + 7), printable_ascii(mmu.read(addr)),
                                    printable_ascii(mmu.read(addr + 1)), printable_ascii(mmu.read(addr + 2)), printable_ascii(mmu.read(addr + 3)),
                                    printable_ascii(mmu.read(addr + 4)), printable_ascii(mmu.read(addr + 5)), printable_ascii(mmu.read(addr + 6)),
                                    printable_ascii(mmu.read(addr + 7)));
                        addr += 0x8;
                    }
                }
                ImGui::EndChild();
                    ...
                    while(clipper.Step()) {
                        ...
                        for(int idx = clipper.DisplayStart; idx < clipper.DisplayEnd; ++idx) {
                            ...
                            if(addr < analyser.labels.size() && analyser.labels[addr])
                                ImGui::Text("%s: 0x%04X 0x%02X [%d] %s", analyser.labels[addr].name.c_str(), addr, mmu.read(addr),
                                            static_cast<int>(cpu.instr_length[mmu.read(addr)]), cpu.get_disassembly(addr).c_str());
                            else
                                ImGui::Text("      0x%04X 0x%02X [%d] %s", addr, mmu.read(addr), static_cast<int>(cpu.instr_length[mmu.read(addr)]),
            ...
            if(ImGui::CollapsingHeader("Sound")) {
                if(snd_buffer.get_buffer() != nullptr) {
                    ImGui::PlotHistogram(
                        "Visualizer", [](void* data, int idx) { return static_cast<float>(static_cast<blip_sample_t*>(data)[idx]); }, snd_buffer.get_buffer(),
                        snd_buffer.buffer_size, 0, nullptr, FLT_MAX, FLT_MAX, ImVec2(0, 100));
                }
            }
            ImGui::End();
        }
    ...
    void handle_event(sf::Event event) {
        ...
        else if(event.type == sf::Event::KeyPressed && !(ImGui::GetIO().WantCaptureKeyboard || ImGui::GetIO().WantTextInput)) {
    ...
    void update_vsync() { window.setVerticalSyncEnabled(real_speed && timing == VSync); }
    ...
    void load_empty_rom() {
        ...
        constexpr unsigned char logo[3 * 16] = {0x00, 0x00, 0x07, 0xc7, 0x09, 0x19, 0x0f, 0x8e, 0x04, 0x67, 0x06, 0x66, 0x0f, 0xcf, 0x08, 0xd9,
    ...
    void setup_window() {
        if(fullscreen)
            window.create(sf::VideoMode::getDesktopMode(), "SenBoy", sf::Style::None); // Borderless Fullscreen : Way easier.
        else
    ...
    void get_frame_input() {
        if(use_movie) {
            switch(movie_type) {
                case SBM: movie.read(playback, 1); break;
                case VBM: movie.read(playback, 2); break;
                case BK2: {
                    char line[13];
                    ...
                    if(line[1] != '.')
                        playback[0] |= 0x40; // Up
                    if(line[2] != '.')
                        playback[0] |= 0x80; // Down
                    if(line[3] != '.')
                        playback[0] |= 0x20; // Left
                    if(line[4] != '.')
                        playback[0] |= 0x10; // Right
                    if(line[5] != '.')
                        playback[0] |= 0x08; // Start
                    if(line[6] != '.')
                        playback[0] |= 0x04; // Select
                    if(line[7] != '.')
                        playback[0] |= 0x02; // B
                    if(line[8] != '.')
                        playback[0] |= 0x01; // A
    ...
    void load_movie(const char* movie_path) {
        ...
        switch(movie_type) {
            ...
            case BK2: // BK2 (Not the archive, just the Input Log)
                ...
                break;
            case Unknown:
            default: log("Error: Unknown movie type."); break;
        }
        if(!movie)
