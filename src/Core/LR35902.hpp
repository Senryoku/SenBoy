...
class LR35902 {
  public:
    ...
    explicit LR35902(const LR35902& rhs) { *this = rhs; }
  ...
  private:
    ...
    inline void set_hl(addr_t val) {
        _h = (val >> 8) & 0xFF;
        _l = val & 0xFF;
    }
    inline void set_de(addr_t val) {
        _d = (val >> 8) & 0xFF;
        _e = val & 0xFF;
    }
    inline void set_bc(addr_t val) {
        _b = (val >> 8) & 0xFF;
        _c = val & 0xFF;
    }
    inline void set_af(addr_t val) {
        _a = (val >> 8) & 0xFF;
        _f = val & 0xF0;
...
inline std::string LR35902::get_disassembly(addr_t addr) const {
    ...
    if(length > 2) {
        if((p = r.find(" a16")) != std::string::npos)
            r.insert(p + 4, " = " + Hexa(_mmu->read16(operand)).str());
        if((p = r.find(",a16")) != std::string::npos)
            r.insert(p + 4, " = " + Hexa(_mmu->read16(operand)).str());
        if((p = r.find("(a16)")) != std::string::npos)
            r.insert(p + 5, " = " + Hexa8(read(_mmu->read16(operand))).str());
        if((p = r.find("d16")) != std::string::npos)
            r.insert(p + 3, " = " + Hexa(_mmu->read16(operand)).str());
    } else if(length > 1 && op != 0xCB) {
        if((p = r.find("a8")) != std::string::npos)
            r.insert(p + 2, " = " + Hexa(0xFF00 + _mmu->read(operand)).str() + " => " + Hexa8(_mmu->read(0xFF00 + _mmu->read(operand))).str());
        if((p = r.find("d8")) != std::string::npos)
            r.insert(p + 2, " = " + Hexa8(_mmu->read(operand)).str());
        if((p = r.find("r8")) != std::string::npos)
            r.insert(p + 2, " = " + Hexa8(from_2c_to_signed(_mmu->read(operand))).str());
    }
    if((p = r.find("(BC)")) != std::string::npos)
        r.insert(p + 4, " = " + Hexa8(_mmu->read(get_bc())).str());
    if((p = r.find("(DE)")) != std::string::npos)
        r.insert(p + 4, " = " + Hexa8(_mmu->read(get_de())).str());
    if((p = r.find("(HL)")) != std::string::npos)
        r.insert(p + 4, " = " + Hexa8(_mmu->read(get_hl())).str());
    if((p = r.find("(HL+)")) != std::string::npos)
        r.insert(p + 5, " = " + Hexa8(_mmu->read(get_hl())).str());
    if((p = r.find("(HL-)")) != std::string::npos)
        r.insert(p + 5, " = " + Hexa8(_mmu->read(get_hl())).str());
    return r;
};
...
inline word_t LR35902::read(addr_t addr) const {
    return in_range(addr, Gb_Apu::start_addr, Gb_Apu::end_addr) ? _apu->read_register(frame_cycles, addr) : _mmu->read(addr);
}
