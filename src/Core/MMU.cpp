...
{
    if(cgb_mode())
        std::memcpy(_mem, gbc_bios, 4096);
    else
        std::memcpy(_mem, gb_bios, 256);

    write(0xFF50, word_t(0x00)); // Enable BIOS ROM
}

bool MMU::load_bios(const std::string& path)
{
...
const word_t MMU::gbc_bios[4096] = {
