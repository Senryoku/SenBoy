...
GPU::GPU(MMU& mmu) : _mmu(&mmu), _screen(new color_t[ScreenWidth * ScreenHeight]) {}

GPU::GPU(const GPU& gpu) : _screen(new color_t[ScreenWidth * ScreenHeight]) {
    *this = gpu;
}
...
struct Sprite {
    ...
    Sprite(word_t _idx = 0, int _x = 0, int _y = 0) : idx(_idx), x(_x), y(_y) {}

    inline bool operator<(const Sprite& s) const { return x < s.x || idx < s.idx; }
};
    ...
    if(get_lcdc() & OBJDisplay) {
        ...
        for(word_t s = 0; s < 40; s++) {
            ...
            if(y <= line && (y + size) > line)
                sprites.emplace_back(s, _mmu->read(0xFE00 + s * 4 + 1) - 8, y);
        }
        ...
        for(const auto& s : sprites) {
            ...
            if(s.x > -8 && s.x < ScreenWidth) {
                ...
                if(get_lcdc() & OBJSize)
                    Tile &= 0xFE; // Bit 0 is ignored for 8x16 sprites
                if(s.y - line >= 8 && (Opt & YFlip))
                    Tile &= 0xFE;
                ...
                for(word_t x = (s.x >= 0 ? 0 : -s.x); x < right_limit; x++) {
                    ...
                    bool   over_bg = (!line_bg_priorities[s.x + x] && // (CGB Only - BG Attributes)
                                    !(Opt & Priority)) ||
                                   line_color_idx[s.x + x] == 0; // Priority over background or transparency

                    if(color != 0 &&                       // Transparency
                       (bg_window_no_priority || over_bg)) // Priority
                    {
                        _screen[to1D(s.x + x, line)] = _mmu->cgb_mode() ? _mmu->get_sprite_color((Opt & PaletteNumber), color) : color_t{Colors[(palette >> (color << 1)) & 3]};
                    }
                }
            }
        }
