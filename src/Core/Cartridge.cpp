...
bool Cartridge::load(const std::string& path) {
    ...
    _data.assign(std::istreambuf_iterator<byte_t>(file), std::istreambuf_iterator<byte_t>());
...
bool Cartridge::init() {
    ...
    if(Log) {
        Log(" Title: '" + getName() + "', Size: " + std::to_string(_data.size()) + "B (" + Hexa8(*(_data.data() + ROMSize)).str() + "), RAM Size: " + std::to_string(_ram_size) +
            "B (" + Hexa8(*(_data.data() + RAMSize)).str() + "), Type: " + Hexa8(getType()).str() + ", Battery: " + (hasBattery() ? "Yes" : "No"));
    ...
    if(_ram_size > 0) {
        ...
        if(hasBattery() && file_exists(save_path())) {
            ...
            _ram.assign(std::istreambuf_iterator<byte_t>(save), std::istreambuf_iterator<byte_t>());
            if(Log)
                Log("Done.");
        } else {
            _ram.clear();
...
void Cartridge::write(addr_t addr, byte_t value) {
    ...
    switch(addr & 0xF000) {
        // External RAM Switch
        case 0x0000:
        case 0x1000: _enable_ram = ((value & 0x0F) == 0x0A); break;
        ...
        case 0xB000:
            ...
            break;
        default: Log("Error: Write on Cartridge on " + Hexa(addr).str()); break;
    }
}
